...
//            ->select();
        ...
        return $this->with(['goods.image', 'address', 'express'])
            ->alias('order')
            ->field('order.*')
            ->join('user', 'user.user_id = order.user_id')
            ->where($dataTypeFilter)
    ...
    public function getChangedDays(){
        $cha=Db::name('integrals')
            ->where('id',1)
            ->field('setday,charges,copyright,is_give')
            ->select()
            ->toArray();
        return $cha;
    }
    ...
    /**
     * 查询订单详情
     * all全部订单 pay待付款 complete已完成 cancel已取消
     */
    public function orderListArr($dataType){

        if($dataType=='pay'){
            //查询待付款订单
            $arrList=Db::name('order')
                ->alias('o')
                ->join('yoshop_user u','o.user_id=u.user_id')
                ->join('yoshop_goods g','o.goods_id=g.goods_id')
