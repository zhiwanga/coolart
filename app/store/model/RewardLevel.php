...
use app\common\model\RewardLevel as RewardLevelModel;
 ...
 */
class RewardLevel extends RewardLevelModel
{
    // 表单验证场景: 新增
...
//        }
    ...
    private function validateForm(array $data, string $scene = self::FORM_SCENE_ADD)
    {
        if ($scene === self::FORM_SCENE_ADD) {
            // 需要判断等级权重是否已存在
            if (self::checkExistByCategoryId($data['category_id'])) {
                $this->error = '等级权重已存在';
                return false;
            }
        } elseif ($scene === self::FORM_SCENE_EDIT) {
            // 需要判断等级权重是否已存在
            if (self::checkExistByCategoryId($data['category_id'], $this['grade_id'])) {
                $this->error = '等级权重已存在';
                return false;
            }
        }
