...
//        var_dump($order['payType']);die;
     ...
     * 创建订单
     */
    public function addorder($userid,$goodsid,$orderNo,$isBox,$boxMoneys,$payType){
        //设置默认的商品数量
        $goodssum=1;
...
//            ];
            ...
            //查询购买盲盒的价格数量
            $boxHowMach=Db::name('integrals')
                ->field('box_money')
                ->where('id',1)
                ->find();
            $boxMach=$boxHowMach['box_money'];
            //填充需要的数据
            $orderGoods=[
                'goods_id'  =>  $goodsid,
                'goods_name'=> $boxMach,
            ...
            $goodsarr=[
                ...
                'order_price'    =>(int)$goodlist['goods_price_max']*$goodssum,
                'pay_price'     =>$goodlist['goods_price_max'],
                'pay_status'     =>10,//默认为未付款
                'order_status'   =>10,//默认为进行中
                'user_id'        =>$userid,
                'store_id'       =>10001,
...
//            ->update();
     ...
     * 查询我的订单，模型层
     */
    public function myOrderFind($userid,$orderType){

        if($orderType==10){
            $myorderlist=DB::name('order')
            ->distinct(true)
                ->alias('o')
                ->leftJoin('yoshop_order_goods yog','o.order_id=yog.order_id')
                ->leftJoin('yoshop_upload_file yuf','yog.image_id=yuf.file_id')
                ->field('o.order_no,o.create_time,o.total_price,yog.goods_name,yuf.file_path,o.pay_status,o.is_box,o.order_status')
                ->where('o.user_id',$userid)
                ->where('o.is_box',0)
                ->where('o.pay_status',$orderType)
                ->where('o.order_status',10)
                ->order('o.create_time','desc')
                ->select()->toArray();
                foreach($myorderlist as $k => $v){
                    $myorderlist[$k]['orderType']=10;
                }
                return $myorderlist;
            ...
            ->distinct(true)
                ...
                foreach($myorderlist as $k => $v){
                    if($myorderlist[$k]['order_status']==10){
                        $myorderlist[$k]['orderType']=10;
                    }else if($myorderlist[$k]['order_status']==20){
                        $myorderlist[$k]['orderType']=30;
                    }else if($myorderlist[$k]['order_status']==30){
                        $myorderlist[$k]['orderType']=20;
                    }

                    //如果盲盒是待付款状态则不输出
                    if($myorderlist[$k]['is_box']==1 and $myorderlist[$k]['pay_status']==10){
                        continue;
                    }
                    
                    $arrs[$k]=$myorderlist[$k];
                }
                return $arrs;
        }
...
//        }
        ...
        $myExtenMan=Db::name('user')
            ->alias('u')
            ->join('yoshop_user_head_img uf','u.user_id = uf.user_id')
            ->where('u.extension_id',$userid)
            ->field('u.user_id,u.nick_name,u.create_time,uf.head_path')
            ->select()
            ->toArray();
        $arr=[
     ...
     **/
     public function getOrder($orderNo){
         $total_price=Db::name('order')->field('total_price')->where('order_no',$orderNo)->find();
         return $total_price;
     }
